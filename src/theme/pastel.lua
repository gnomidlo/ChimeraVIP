...
function UI:install_theme()
    ...
    local P = self.colors
    local theme = scripts.ui.themes.plain:new()
    theme._chimera_vip_pastel = true
...
QToolBar { background-color: ]]..P.background..[[; background-image: none; border: 0px; border-bottom: 1px solid ]]..P.separator..[[; }]])
    ...
    function theme:get_footer_stylesheet()
        return "background-color:"..P.background..";background-image:none;border:0px;border-top:1px solid "..P.separator..";"
    end
    function theme:get_window_stylesheet()
        return "QWidget { padding:4px; background-color:"..P.background.."; background-image:none; color:"..P.text.."; }"
    end
    function theme:get_button_window_stylesheet(height)
        return string.format("QWidget { padding:%dpx 4px 4px 4px; background-color:%s; background-image:none; color:%s; }", height + 4, P.background, P.text)
    end
    function theme:get_button_area_bg()
        return "background-color:"..P.background_soft..";background-image:none;border:0px;border-bottom:1px solid "..P.separator..";"
    end
    function theme:get_border_stylesheet()
        return "background:none;border:1px solid "..P.separator..";"
    end
    function theme:get_notification_color() return closestColor(P.lavender) end
    function theme:get_notification_stylesheet()
        return "QLabel { color:"..P.text.."; padding:7px 10px; border:1px solid "..P.separator.."; border-radius:5px; background-color:"..P.background_soft.."; qproperty-wordWrap:true; }"
    end
    function theme:get_notification_close_stylesheet()
        return "QLabel { background-color:transparent;color:"..P.text_muted..";border:0px;} QLabel:hover {color:"..P.rose..";}"
    end
    function theme:get_button_stylesheet(_, font_size)
        return "QLabel {border:1px solid "..P.separator..";border-radius:3px;padding:0px 5px;background-color:"..P.background_soft..";background-image:none;color:"..P.text..";font-size:"..tostring(font_size).."px;} QLabel:hover {border-color:"..P.lavender..";color:"..P.lavender..";}"
    end
...
function UI:select_theme()
    ...
    scripts.ui.states_font_size = 7
    return true
end

function UI:is_current()
    return scripts and scripts.ui and scripts.ui.theme == self.theme_name
        and scripts.ui.current_theme and scripts.ui.current_theme._chimera_vip_pastel == true
end

function UI:style_console(name, font_size)
    local br,bg,bb = hex_rgb(self.colors.background)
    local fr,fg,fb = hex_rgb(self.colors.text)
    pcall(setBackgroundColor, name, br,bg,bb,255)
    pcall(setFgColor, name, fr,fg,fb)
    if font_size then pcall(setFontSize, name, font_size) end
end

function UI:style_known_windows()
    self:style_console("states_window",7); self:style_console("enemy_states_window",7)
...
function UI:finish_ready()
    ...
    self:style_known_windows()
    C.theme_ready = true
    chimera_overlay.theme_ready = true
    raiseEvent("chimeraThemeReady", self.theme_name)
    return true
end
...
function UI:apply()
    ...
    local ok, err = pcall(function() scripts.ui:setup() end)
    self.applying = false
