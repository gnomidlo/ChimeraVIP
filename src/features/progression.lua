...
function P:on_stats(snapshot)
    ...
    if not record.last_observed then
        ...
        self:append_history(record, {
            time = current.captured_at or os.time(),
            kind = "baseline",
            snapshot = copy_table(current),
            diff = {},
            xp_since_previous = 0,
        })
        self:schedule_save()
        raiseEvent("chimeraVipProgressionUpdated", record.display_name, "baseline", record)
        return
    end
    ...
    -- Samo sprawdzanie cech bez wzrostu nie zeruje licznika XP.
    if not changed then
        self:schedule_save()
        raiseEvent("chimeraVipProgressionUpdated", record.display_name, "observed", record)
        return
    end
    ...
    self:append_history(record, {
        ...
        snapshot = copy_table(current),
        diff = diff,
        xp_since_previous = spent,
    })

    record.baseline = copy_table(current)
    record.xp_since_change = 0
    self:schedule_save()
    raiseEvent("chimeraVipProgressionUpdated", record.display_name, "change", record, diff, spent)
end
...
local function diff_text(diff, Cc)
    ...
    for _, key in ipairs(order) do
        ...
        if value ~= 0 then
            local color = value > 0 and Cc.mint or Cc.rose
            parts[#parts + 1] = color .. key .. string.format(" %+d", value)
        end
    end
...
function P:show_info()
    ...
    if not record or not record.last_observed then
        hecho("\n" .. Cc.lavender .. "PROGRES — " .. display
            .. "\n" .. Cc.text_muted .. "Brak snapshotu cech. Wyswietl cechy w grze, aby zalozyc punkt bazowy.")
        return
    end

    local s = record.last_observed
    local st = s.stats or {}
    local last_change = nil
    for i = #(record.history or {}), 1, -1 do
        if record.history[i].kind == "change" then last_change = record.history[i]; break end
    end

    hecho("\n\n" .. Cc.lavender .. "PROGRES — " .. tostring(record.display_name)
        .. "\n" .. Cc.separator .. "--------------------------------------------------"
        .. "\n" .. Cc.text_muted .. "Sil " .. Cc.text .. tostring(st.Sil or "-")
        .. Cc.text_muted .. " | Zr " .. Cc.text .. tostring(st.Zr or "-")
        .. Cc.text_muted .. " | Wt " .. Cc.text .. tostring(st.Wt or "-")
        .. Cc.text_muted .. " | Int " .. Cc.text .. tostring(st.Int or "-")
        .. Cc.text_muted .. " | Md " .. Cc.text .. tostring(st.Md or "-")
        .. Cc.text_muted .. " | Odw " .. Cc.text .. tostring(st.Odw or "-")
        .. "\n" .. Cc.text_muted .. "Fiz " .. Cc.blue .. tostring(s.physical or 0)
        .. Cc.text_muted .. " | Ment " .. Cc.lavender .. tostring(s.mental or 0)
        .. Cc.text_muted .. " | Odw " .. Cc.yellow .. tostring(s.courage or 0)
        .. Cc.text_muted .. " | Łącznie " .. Cc.peach .. tostring(s.total or 0)
        ...
        .. (last_change and (Cc.text_muted .. "\nOstatnia zmiana " .. Cc.text .. os.date("%Y-%m-%d %H:%M", last_change.time)
            .. Cc.text_muted .. "\n  " .. diff_text(last_change.diff, Cc)
...
function P:show_history(count)
    ...
    if not display then hecho("\n" .. Cc.rose .. "[PROGRES] Brak danych gmcp.Char.Name."); return end
    if not record or #(record.history or {}) == 0 then
    ...
    for i = first, #record.history do
        local entry = record.history[i]
        local label = entry.kind == "baseline" and "punkt bazowy" or diff_text(entry.diff, Cc)
        hecho("\n" .. Cc.text_muted .. os.date("%Y-%m-%d %H:%M", entry.time)
            .. "  " .. label
            .. (entry.kind == "change" and (Cc.text_muted .. "  | XP " .. Cc.peach .. fmt_int(entry.xp_since_previous or 0)) or ""))
    end
end
...
function P:show_characters()
    ...
    local list = {}
    for _, record in pairs(self.data.characters or {}) do list[#list + 1] = record end
    table.sort(list, function(a, b) return tostring(a.display_name) < tostring(b.display_name) end)
    if #list == 0 then hecho("\n" .. Cc.text_muted .. "Brak zapisanych postaci."); return end
    for _, record in ipairs(list) do
        hecho("\n" .. Cc.mint .. tostring(record.display_name or record.key)
            .. Cc.text_muted .. "  XP " .. Cc.peach .. fmt_int(record.tracked_xp or 0)
            .. Cc.text_muted .. "  zmian " .. Cc.text .. tostring(math.max(0, #(record.history or {}) - 1)))
    end
end

function P:help()
    local Cc = colors()
    hecho("\n\n" .. Cc.lavender .. "PROGRES — KOMENDY"
        .. "\n" .. Cc.text .. "/progres" .. Cc.text_muted .. "                 aktualna postac"
        .. "\n" .. Cc.text .. "/progres historia [N]" .. Cc.text_muted .. "    ostatnie zmiany"
        .. "\n" .. Cc.text .. "/progres postacie" .. Cc.text_muted .. "         zapisane postacie"
        .. "\n" .. Cc.text .. "/cechy info" .. Cc.text_muted .. "              alias do /progres"
        .. "\n" .. Cc.text .. "/cechy historia [N]" .. Cc.text_muted .. "      alias historii")
end

-- Hot reload aliases/handlers.
...
P.alias_ids[#P.alias_ids + 1] = tempAlias([[^/progres help$]], function() P:help() end)
