...
-- Wyjatki klasyfikacji sa czescia ChimeraVIP, nie lokalnych danych uzytkownika.
-- Kolejne niestandardowe moby dopisujemy tutaj wraz z wersja skryptu.
XP.rules = {
...
local function normalize(text) return string.lower(trim(text):gsub("%s+", " ")) end
...
function XP:add_event(raw_mob, amount, killer, own)
    ...
    S.events[#S.events + 1] = {time=now,xp=amount,mob_raw=trim(raw_mob),mob=mob,classification=classification,killer=killer,own=own==true}
...
function XP:get_window_stats(from_time, to_time)
    ...
    for _, event in ipairs(self.session.events) do if event.time >= from_time and event.time <= to_time then amount=amount+event.xp; kills=kills+1 end end
    return amount, kills
end
...
function XP:get_mob_stats()
    ...
    for _,event in ipairs(self.session.events) do
        local key=event.mob
        stats[key]=stats[key] or {name=key,xp=0,kills=0,builtin=event.classification=="rule",own_kills=0,group_kills=0}
        local data=stats[key]; data.xp=data.xp+event.xp; data.kills=data.kills+1
    ...
    local list={}; for _,data in pairs(stats) do data.average=data.xp/data.kills; list[#list+1]=data end
    table.sort(list,function(a,b) if a.xp==b.xp then return a.kills>b.kills end; return a.xp>b.xp end)
    return list
end
function XP:find_mob(name) name=normalize(name); for _,data in ipairs(self:get_mob_stats()) do if data.name==name then return data end end; return nil end
...
function XP:show_last(count)
    count=math.max(1,math.min(100,tonumber(count) or 10)); local Cc,events=self.colors,self.session.events
...
function XP:reset()
    self.session={started_at=nil,last_kill_at=nil,active_seconds=0,total_xp=0,kills=0,own_kills=0,group_kills=0,own_xp=0,group_xp=0,events={}}
...
function XP:show_help()
    ...
    for _,row in ipairs(rows) do
        local w=U and U.text_width and U.text_width(row[1]) or #row[1]
        command_width=math.max(command_width,w)
    end
