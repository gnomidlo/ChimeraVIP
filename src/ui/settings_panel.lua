...
function SP:open()
    self:destroy()
    local c=P(); local pw,ph=458,144; local x,y=self:panel_position(pw,ph); local prefix="chimera_vip.settings_panel."..tostring(os.time()).."."..tostring(math.random(1000,9999))
    ...
    local title=Geyser.Label:new({name=prefix..".title",x=14,y=8,width=390,height=22,fontSize=10},self.root); title:setStyleSheet(U.transparent_css()); title:echo("<font color='"..c.lavender.."'>CHIMERAVIP — USTAWIENIA</font>")
    local close=Geyser.Label:new({name=prefix..".close",x=421,y=6,width=25,height=22,fontSize=11},self.root); close:setStyleSheet("QLabel {background-color:transparent;color:"..c.text_muted..";border:0px;padding:0px;} QLabel:hover {color:"..c.rose..";}"); close:echo("<center>×</center>"); close:setClickCallback(function() SP:destroy() end); pcall(setLabelCursor,close.name,"PointingHand")
    local subtitle=Geyser.Label:new({name=prefix..".subtitle",x=14,y=34,width=420,height=18,fontSize=8},self.root); subtitle:setStyleSheet(U.transparent_css()); subtitle:echo("<font color='"..c.text_muted.."'>Rozmiar tekstu okna stanów (drużyna / wrogowie / inni) — kliknij:</font>")
    local sizes={8,9,10,11,12,13,14}; local start_x,gap,bw=14,7,54
    ...
    local ml=Geyser.Label:new({name=prefix..".module_label",x=14,y=110,width=230,height=22,fontSize=8},self.root); ml:setStyleSheet(U.transparent_css()); ml:echo("<font color='"..c.text_muted.."'>Moduły:</font> <font color='"..c.text.."'>Kolory walki</font>")
    self.module_button=Geyser.Label:new({name=prefix..".combat_colors",x=331,y=106,width=113,height=28,fontSize=9},self.root); self.module_button:setClickCallback(function() SP:toggle_combat_colors() end); pcall(setLabelCursor,self.module_button.name,"PointingHand")
